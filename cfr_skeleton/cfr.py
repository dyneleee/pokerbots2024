...
class CFR:
    ...
    def calculate_pot(self, history):
        ...
        for rd_history in history:
            for i in range(len(rd_history)):
                bet_amt = 0
                if rd_history[i] == 1:
                    bet_amt = rd_history[i-1]
                elif rd_history[i] == 2:
                    bet_amt = int(pot/3)
                elif rd_history[i] == 3:
                    bet_amt = int(pot/2)
        ...
        return (pot, stack_0, stack_1)
 
    def valid_bets(self, history, rd, acting_player): #check = 0, call = 1, 1/3 pot = 2, 1/2 pot 3, pot = 4, overbet
        calc = self.calculate_pot(history) # (pot, stack_0, stack_1)
        if acting_player == 0:
            acting_stack = calc[1]
            opp_stack = calc[2]
            #acting_stack = int(400 - (np.sum(history[0][0::2]) + np.sum(history[1][0::2])))
        elif acting_player == 1:
            acting_stack = calc[2]
            opp_stack = calc[1]
            #acting_stack = int(400 - (np.sum(history[0][1::2]) + np.sum(history[1][1::2])))
        ...
        max_raise = min(acting_stack, opp_stack)
        pot = calc[0]
        bets = [0]
        ...
        if len(history[rd]) == 0:
            ...
            #bets.append(0)
            if int(pot/3) <= max_raise:
                bets.append(2)
            if int(pot/2) <= max_raise:
                bets.append(3)
            if int(pot) <= max_raise:
                bets.append(4)
            ...
            if history[rd] == [0]:
                ...
                if int(pot/3) <= max_raise and int(pot/3) >= min_raise:
                    bets.append(2)
                if int(pot/2) <= max_raise and int(pot/2) >= min_raise:
                    bets.append(3)
                if int(pot) <= max_raise and int(pot) >= min_raise:
                    bets.append(4)
